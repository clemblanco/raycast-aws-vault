...
export default function Command() {
  ...
  const logout = async () => {
    const browserUserDataDir = resolve(homedir(), `.aws/chrome`);
    execSync(`rm -rf ${browserUserDataDir}`);
...
function ProfileListItem({ profile }: { profile: Profile }) {
  const onClick = async () => {
    ...
    const profileDirName = profile.name.replace(/^a-zA-Z0-9_-/i, "__");
    const userDataDir = resolve(homedir(), `.aws/chrome/${profileDirName}`);
    execSync(`mkdir -p ${userDataDir}`);
    const diskCacheDir = execSync(`mktemp -d /tmp/aws_chrome_cache.XXXXXXXX`).toString().trim();
